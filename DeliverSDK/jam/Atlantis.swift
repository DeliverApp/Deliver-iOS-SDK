...
import ObjectiveC
//
//internal protocol AtlantisDelegate: AnyObject {
//
...
internal final class Atlantis: NSObject {
...
//    private weak var delegate: AtlantisDelegate?
    private let transporters: [Transporter]
    private var injector: Injector = NetworkInjector()
    private(set) var configuration: JamConfiguration = JamConfiguration.default()
    private var packages: [String: TrafficPackage] = [:]
    private lazy var waitingWebsocketPackages: [String: [TrafficPackage]] = [:]
    private let queue = DispatchQueue(label: "com.deliver.jam")
...
//            ,ConsoleTransporter()
    ...
    internal static let buildVersion: String = "1.23.0"
    ...
    @objc internal class func start(hostName: String? = nil, shouldCaptureWebSocketTraffic: Bool = true) {
        // save config
        let configuration = JamConfiguration.default(hostName: hostName)
        ...
        if Atlantis.shared.isEnabledTransportLayer {
            Atlantis.shared.transporters.forEach { $0.start(configuration) }
        }
    }

    /// Stop monitoring
    @objc internal class func stop() {
        guard isEnabled.value else { return }
        isEnabled.mutate { $0 = false }
        if Atlantis.shared.isEnabledTransportLayer {
            Atlantis.shared.transporters.forEach { $0.stop() }
        }
    }

    /// Enable Transport Layer (e.g. Bonjour)
    internal class func setEnableTransportLayer(_ isEnabled: Bool) {
        Atlantis.shared.isEnabledTransportLayer = isEnabled
    }

    /// Enable Swift Playground mode
    internal class func setIsRunningOniOSPlayground(_ isEnabled: Bool) {
        Atlantis.shared.isRunningOniOSPlayground = isEnabled
    }

//    /// Set delegate to observe the traffic
//    internal class func setDelegate(_ delegate: AtlantisDelegate) {
    ...
    private func getPackage(_ taskOrConnection: AnyObject) -> TrafficPackage? {
        ...
        case let task as URLSessionTask:
            guard let package = TrafficPackage.buildRequest(sessionTask: task, id: id) else {
                print("[DeliverJam] ❌ Error: Should build package from URLSessionTask")
                return nil
            }
            packages[id] = package
            return package
        default:
            print("[DeliverJam] ❌ Error: Do not support new Type \(String(describing: taskOrConnection.className))")
        }
        return nil
    }
}
    ...
    private func prepareAndSendWSMessage(task: URLSessionTask, wsPackageBuilder: (String) -> WebsocketMessagePackage?) {
        ...
        if let package = packages[id] {
            ...
            guard let wsPackage = wsPackageBuilder(id) else {
                print("[DeliverJam][Error] Skipping sending WS Packages!! Please contact Deliver Team.")
                return
            }
...
*/
        ...
        transporters.forEach{ $0.send(package: message) }
    }
