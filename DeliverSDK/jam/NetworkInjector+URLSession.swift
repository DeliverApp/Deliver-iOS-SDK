...
func logError(name: String) {
    print("❌ [DeliverJam] Could not swizzle this func: \(name)! It looks like the latest iOS (beta) has changed, please contact support")
}
...
extension NetworkInjector {
    ...
    func _swizzleURLSessionWebsocketSelector() {
        guard let websocketClass = NSClassFromString("__NSURLSessionWebSocketTask") else {
            print("[logError][ERROR] Could not inject __NSURLSessionWebSocketTask!!")
            return
        }
