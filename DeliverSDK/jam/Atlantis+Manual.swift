...
extension Atlantis {
    ...
    internal class func add(request: URLRequest,
        ...
        guard let package = TrafficPackage.buildRequest(urlRequest: request, urlResponse: response, bodyData: responseBody) else {
            print("[DeliverJam][Error] Could not build TrafficPackage from manual input. Please contact the author!")
            return
        }
    ...
    ///   - responseBody: The body data of the response
    internal class func add(request: JamRequest,
                          response: JamResponse,
                          responseBody: Data?) {
        // Build package from raw given input
    ...
    internal class func addGRPCUnary(path: String,
                            requestObject: Data?,
                            responseObject: Data?,
                            success: Bool,
                            statusCode: Int,
        ...
        let request = JamRequest(url: path, method: "GRPC", headers: HPACKHeadersRequest, body: requestObject)
        ...
        let response = JamResponse(statusCode: success ? 200 : 503, headers: headers)
    ...
    ///   - HPACKHeadersRequest: Transformed request headers from gRPC. Get it from `callOptions?.customMetadata`
    ///   - HPACKHeadersResponse: Transformed response headers from gRPC. Get it from `CallResult.trailingMetadata` or `CallResult.initialMetadata`
    internal class func addGRPCStreaming(id: UUID,
                                path: String,
                                ...
                                HPACKHeadersRequest: [Header] = [],
                                HPACKHeadersResponse: [Header] = []) {
        let request: JamRequest
        switch streamingType
        {
        case .client:
            request = JamRequest(url: path, method: "GRPC", headers: HPACKHeadersRequest, body: nil)
        case .server:
            request = JamRequest(url: path, method: "GRPC", headers: HPACKHeadersRequest, body: message.optionalData)
        case .bidirectional:
            request = JamRequest(url: path, method: "GRPC", headers: HPACKHeadersRequest, body: nil)
        }
        // Wrap the CallResult to Response Headers
    ...
    internal enum GRPCStreamingType {
        case client
        case server
        case bidirectional
    }
